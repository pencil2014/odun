    ...
    <div class="success-box content">
      <div class="result">
        <div class="qrcode-box">
          ...
          <div class="code">
            <h3>{{type | formatType}}跑团成功</h3>
            <p class="sub"><span>{{second}}S</span> 后将自动跳转到 <nuxt-link :to="'/user/rungroup-info?groupId='+ groupId">跑团详情</nuxt-link></p>
            <p class="name">跑团名称：{{name}}</p>
...
export default {
  ...
  methods: {
    ...
    countDown () {
      ...
      if (this.second > 0) {
        ...
        this.$router.push({path:'/user/rungroup-info', query:{groupId: this.groupId}})
        this.second = 60
      }
    },
    copylink () {
  ...
  mounted () {
    ...
    this.imgUrl = process.env.baseUrl + `dist/run-group-detail.html?groupId=${this.groupId}&pageName=runGroupDetail&useWhere=wx`
    this.countDown()
  },
  beforeDestroy () {
